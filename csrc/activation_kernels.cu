...
__global__ void silu_and_mul_kernel(
  ...
  const int d) {
  const int64_t token_idx = blockIdx.x; // 每个线程块处理一个 token 的所有维度
  // 每个线程处理一个token 的部分维度，
  // 例如 blockIdx.x 为 0， blockDim.x 为 4，d 为 128，
  // threadIdx.x 为 0 的线程处理 0*2*128 + 0, 0*2*128 + 4, 0*2*128 + 8, ..., 0*2*128 + 124 等维度
  // threadIdx.x 为 1 的线程处理 0*2*128 + 1, 0*2*128 + 5, 0*2*128 + 9, ..., 0*2*128 + 125 等维度
  // threadIdx.x 为 2 的线程处理 0*2*128 + 2, 0*2*128 + 6, 0*2*128 + 10, ..., 0*2*128 + 126 等维度
  // threadIdx.x 为 3 的线程处理 0*2*128 + 3, 0*2*128 + 7, 0*2*128 + 11, ..., 0*2*128 + 127 等维度
