...
class Transformer(nn.Module):
    ...
    def load_weight(self, weights: Dict[str, Any]):
        layers_row_parallel = ["tok_embeddings", "attention.wq", "attention.wk", "attention.wv", "feed_forward.w1", "feed_forward.w3", "output.weight"]
        layers_col_parallel = ["attention.wo", "feed_forward.w2"]
        ...
        for name, loaded_weight in weights.items():
            ...
            weight = None
            for weight_name in layers_col_parallel:
                if weight_name in name:
                    shard_size = param.shape[1]
                    weight = loaded_weight[:,local_rank*shard_size:(local_rank+1)*shard_size]
                    break
            
            for weight_name in layers_row_parallel:
                if weight_name in name: 
                    shard_size = param.shape[0]
                    weight = loaded_weight[local_rank*shard_size:
                                        (local_rank+1)*shard_size, :]
                    break
